...
internal class SwipeableFeedLayout : FrameLayout {
    ...
    private fun moveLayouts(translation: Float) {
        // front
        val frontScale = 1f + 0.2f * abs(translation) / width
        var opacity = abs(translation) / width
        ...
        // back
        var backScale = BACK_LAYOUT_INITIAL_SCALE + (1 - BACK_LAYOUT_INITIAL_SCALE) * abs(translation) / width
    ...
    private fun cancelMove() {
        ...
        if (currTranslation != 0f) {
            if (abs(xVelocity) < minFlingVelocity) {
                ...
                if (abs(currTranslation) < width / 3f) {
                    ...
                    scroller.startScroll(currTranslation.toInt(), 0, (sign(currTranslation) * getSwipeOutMaxTranslation()).toInt(), 0)
                }

            } else {

                var velocity = xVelocity
                if (abs(velocity) > maxFlingVelocity) {
                    velocity = sign(velocity) * maxFlingVelocity.toFloat() * 2f
                } else if (abs(velocity) < maxFlingVelocity / 5f) {
                    velocity = sign(velocity) * maxFlingVelocity / 1.5f
                }
